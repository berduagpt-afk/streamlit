# run_modeling.py (FINAL PATCHED - MODUL ONLY)
...
class ModelingConfig:
    ...
    col_detail: str = "detailed_decription"  # sesuai DDL
...
def ensure_output_tables(engine: Engine, cfg: ModelingConfig) -> None:
    # NOTE: kita biarkan kolom sub_modul tetap ada di summary/members untuk kompatibilitas tabel yang sudah terbuat.
    ddl = f"""
...
def fetch_source_data(engine: Engine, cfg: ModelingConfig) -> pd.DataFrame:
    ...
    # tanggal: gunakan end_date_plus di Python (hindari :end_date::date)
    if cfg.start_date:
    ...
    df["detailed_decription"] = df["detailed_decription"].fillna("").astype(str)
...
def compute_top_terms(vectorizer: TfidfVectorizer, X_seg, local_positions: List[int], topk: int = 12) -> str:
    ...
    mean_vec = np.asarray(Xc.mean(axis=0)).ravel()  # aman (bukan np.matrix)
    if mean_vec.size == 0:
        return ""
...
def pick_representative_pos(X_seg, local_positions: List[int]) -> Optional[int]:
    if not local_positions:
        return None
    Xc = X_seg[local_positions]
    centroid = np.asarray(Xc.mean(axis=0))          # FIX np.matrix -> ndarray
    sims = cosine_similarity(Xc, centroid).ravel()  # cosine_similarity aman
...
def run_modeling(engine: Engine, cfg: ModelingConfig, dry_run: bool = False) -> str:
    ...
    params_json = {
        ...
        "performance": {"full_matrix_limit": cfg.full_matrix_limit, "nn_topk": cfg.nn_topk},
    }
    ...
    data_range = {
        ...
        "site": cfg.site,
        "n_rows": int(len(df)),
        "note": "sub_modul diabaikan untuk grouping; hanya disimpan sebagai metadata pada cluster_members",
    }

    summary_rows = []
    member_rows = []

    # ✅ Segment: modul + window_start (sub_modul diabaikan)
    ...
    for (modul, wstart), g in grouped:
        ...
        for cl in np.unique(labels):
            ...
            # ✅ cluster_id tanpa sub_modul
            cluster_id = f"{modul}__{wstart}__{cluster_counter}"
            cluster_counter += 1

            rep_pos = pick_representative_pos(X_seg, local_positions) or local_positions[0]
            ...
            for _, row in gc.iterrows():
                member_rows.append({
                    ...
                    "modul": str(row["modul"]),
                    "sub_modul": str(row["sub_modul"]),  # tetap disimpan sebagai metadata
    ...
    df_runs = pd.DataFrame([{
        ...
        "data_range": json.dumps(data_range),
        "notes": None,
    }])
