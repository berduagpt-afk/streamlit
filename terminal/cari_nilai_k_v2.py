...
class Config:
    ...
    # ---------- PATCH BARU (OUTPUT) ----------
    # Output file diberi suffix run_id & timestamp agar tidak ketimpa
    output_prefix: str = "knn_sensitivity_results"
    # ---------------------------------------
...
def fetch_tfidf(engine, run_id: str, max_docs: Optional[int]) -> pd.DataFrame:
    """
    PATCH: Tambah ORDER BY agar LIMIT deterministik (reproducible).
    Pilih kolom incident_number tetap disertakan untuk audit/debug.
    """
    limit_sql = f"LIMIT {int(max_docs)}" if max_docs else ""
    q = text(f"""
...
def build_sparse_matrix(df: pd.DataFrame, feature_names: List[str]) -> csr_matrix:
    ...
    # L2 normalization (WAJIB untuk cosine)
    norms = np.sqrt(X.multiply(X).sum(axis=1)).A1
    norms[norms == 0] = 1.0

    # Paksa kembali ke CSR (agar slicing aman & konsisten)
...
def cluster_knn(X: csr_matrix, k: int, sim_threshold: float) -> np.ndarray:
    """
    PATCH: Vectorized edge construction (jauh lebih cepat dibanding nested loop Python).
    """
    nn = NearestNeighbors(
    ...
    # rows: [0,0,0..,1,1,1..] dipilih oleh mask
    rows = np.repeat(np.arange(X.shape[0], dtype=np.int32), mask.shape[1])[mask.ravel()]
    cols = nbh.ravel()[mask.ravel()].astype(np.int32)

    A = csr_matrix(
        (np.ones(rows.size, dtype=np.int8), (rows, cols)),
        shape=(X.shape[0], X.shape[0])
    )
    # buat graph undirected
    A = A.maximum(A.T)
...
def evaluate(X: csr_matrix, labels: np.ndarray) -> Tuple[float, float, float]:
    """
    Return:
    - silhouette (cosine) pada ruang SVD
    - DBI (safe subset) pada ruang SVD
    - svd_explained_sum (indikasi seberapa banyak varians yang tertangkap)
    """
    rng = np.random.default_rng(CFG.seed)

    uniq_all = np.unique(labels)
    if len(uniq_all) < 2:
    ...
    explained_sum = float(np.sum(svd.explained_variance_ratio_)) if hasattr(svd, "explained_variance_ratio_") else np.nan
    ...
    if not keep_clusters:
        ...
        keep_mask = np.isin(ys, list(keep_clusters))
        Z_dbi = Z[keep_mask]
        y_dbi = ys[keep_mask]
    ...
    return float(sil), (float(dbi) if dbi == dbi else np.nan), explained_sum


# ======================================================
# 📈 METRIK TAMBAHAN UNTUK INTERPRETASI
# ======================================================
def cluster_summary_metrics(labels: np.ndarray) -> Dict[str, Any]:
    ...
    median_cluster = float(np.median(cnt)) if len(cnt) else np.nan
    p95_cluster = float(np.percentile(cnt, 95)) if len(cnt) else np.nan

    # share top-10 cluster terbesar (indikasi "menggumpal")
    if len(cnt) > 0:
        top10 = np.sort(cnt)[-10:]
        top10_share = float(top10.sum() / cnt.sum())
    else:
        top10_share = np.nan

    singleton_share = float(n_singletons / n_docs) if n_docs else np.nan
...
def run_knn_experiment():
    ...
    for k in CFG.k_grid:
        ...
        m = cluster_summary_metrics(labels)
        sil, dbi, svd_var = evaluate(X, labels)
        ...
        results.append({
            ...
            **m,
            ...
            # evaluasi internal (aproksimasi via SVD)
            "silhouette": sil,
            "davies_bouldin": dbi,
            "svd_explained_sum": svd_var,
        ...
        print(
            f"k={k:>2} | clusters={m['n_clusters_all']:>6,} | singletons={m['n_singletons']:>6,} "
            f"({m['singleton_share']*100:>5.1f}%) | max={m['max_cluster_size']:>6,} | "
            f"med={m['median_cluster_size']:<6.1f} | top10={m['top10_share']*100:>5.1f}% | "
            f"sil={sil:.4f} | dbi={'NaN' if dbi != dbi else f'{dbi:.4f}'} | svd_var={svd_var:.3f}"
        )

    df_res = pd.DataFrame(results)

    # output file unik (tidak ketimpa)
    ts = pd.Timestamp.now().strftime("%Y%m%d_%H%M%S")
    out_name = f"{CFG.output_prefix}_{run_id}_{ts}.csv"
    df_res.to_csv(out_name, index=False)

    print(f"\n[OK] Hasil disimpan ke {out_name}")
    return df_res
