# run_modeling.py (FINAL PATCHED - FULL)
...
class ModelingConfig:
    ...
    # Insert performance
    insert_chunksize: int = 5000
    insert_method: str = "multi"  # pandas.to_sql method
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--insert-chunksize", type=int, default=5000)
    p.add_argument("--insert-method", default="multi")  # multi / None
...
def ensure_output_tables(engine: Engine, cfg: ModelingConfig) -> None:
    ...
    Plus: memastikan kolom threshold & window_days ada di modeling_runs (ALTER TABLE jika perlu).
    """
    ddl_create = f"""
    ...
    CREATE INDEX IF NOT EXISTS idx_{cfg.runs_table}_approach_thr_win
        ON {cfg.output_schema}.{cfg.runs_table} (approach, threshold, window_days, run_time);
    """
    ...
    with engine.begin() as conn:
        ...
        _exec_ddl_statements(conn, ddl_alter_runs)
...
def _adaptive_topk(n: int, base_topk: int) -> int:
    """Top-k adaptif untuk mengurangi risiko cluster pecah pada kNN."""
    if n <= 2:
        return 1
...
def run_modeling(engine: Engine, cfg: ModelingConfig, dry_run: bool = False) -> str:
    ...
    params_json: Dict[str, Any] = {
        ...
        "performance": {
            "full_matrix_limit": int(cfg.full_matrix_limit),
            "nn_topk_base": int(cfg.nn_topk),
    ...
    for (modul, wstart), g in grouped:
        ...
        for cl in np.unique(labels):
            ...
            summary_rows.append({
                ...
                "top_terms": top_terms,
                # ✅ JSONB aman utk to_sql (string JSON)
    ...
    # ✅ modeling_runs record (JSONB as JSON string for pandas->psycopg2)
    ...
    print(f"[INFO] threshold={cfg.cosine_threshold} window_days={cfg.window_days}")
    ...
    # Insert tuning
    method = cfg.insert_method if cfg.insert_method and cfg.insert_method.lower() != "none" else None
    chunksize = int(cfg.insert_chunksize) if cfg.insert_chunksize else None

    with engine.begin() as conn:
        df_runs.to_sql(cfg.runs_table, conn, schema=cfg.output_schema, if_exists="append",
                      index=False, method=method, chunksize=chunksize)

        if not df_summary.empty:
            df_summary.to_sql(cfg.summary_table, conn, schema=cfg.output_schema, if_exists="append",
                              index=False, method=method, chunksize=chunksize)

        if not df_members.empty:
            df_members.to_sql(cfg.members_table, conn, schema=cfg.output_schema, if_exists="append",
                              index=False, method=method, chunksize=chunksize)
