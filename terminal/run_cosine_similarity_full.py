...
# 3) CSR dirapikan:
#    - X.sum_duplicates()
# 4) Normalisasi L2 dilakukan SEKALI di main (in-place)
# 5) tfidf_run_id disimpan sebagai TEXT (bukan UUID) agar kompatibel dengan run_id string
# ============================================================
...
def compute_cosine_distribution_knn(
    ...
    sim_vals = sim[:, 1:].reshape(-1).astype(np.float32)  # drop self
...
def cluster_text(df: pd.DataFrame, X_normed: csr_matrix, thr: float, knn_k: int) -> pd.DataFrame:
    ...
    PATCH: definisi neighbor konsisten:
