...
def ensure_output_tables(engine: Engine, cfg: Config) -> None:
    ...
    CREATE INDEX IF NOT EXISTS idx_{cfg.out_members}_episode
      ON {cfg.schema}.{cfg.out_members} (job_id, modeling_id, window_days, temporal_cluster_id);
...
def load_members_for_window(engine: Engine, cfg: Config, job_id: str, modeling_id: str, window_days: int) -> pd.DataFrame:
    ...
    PATCH:
    - eligible_clusters menyimpan job_id + modeling_id + cluster_id
    - JOIN memakai ketiganya agar aman bila cluster_id tidak unik global
    """
    sql = text(f"""
...
def sessionize(df: pd.DataFrame, window_days: int) -> pd.DataFrame:
    """
    df harus sudah berisi anggota cluster, sorted by cluster_id, tgl_submit, incident_number.
    Output menambahkan:
      - window_days
      - gap_days (int, hari)
    ...
    # basis hari (date) sesuai narasi tesis
    ...
    out["temporal_cluster_no"] = (
        out.groupby(["job_id", "modeling_id", "cluster_id"])["is_new_episode"].cumsum() + 1
    ).astype(int)
...
def save_sessionized(engine: Engine, cfg: Config, df_out: pd.DataFrame) -> None:
    """
    Idempotent per (job_id, modeling_id, window_days):
    - delete dulu subset tsb lalu insert ulang.
    """
    if df_out.empty:
        return
