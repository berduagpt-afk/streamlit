# pages/load_dataset.py — Statistik Populasi 7 Kolom dari incident_kelayakan
...
def load_from_db(schema="lasis_djp", table="incident_kelayakan") -> pd.DataFrame:
    """Ambil 7 kolom utama dari tabel incident_kelayakan."""
    eng = pg_engine()
    try:
        q = text(
...
def load_data() -> pd.DataFrame:
    # kalau sebelumnya sudah disimpan di session (opsional), pakai itu
...
st.title("📊 Statistik Populasi Dataset (incident_kelayakan)")
...
st.download_button(
    "⬇️ Unduh CSV (incident_kelayakan, 7 kolom)",
    data=buf.getvalue(),
    file_name="dataset_incident_kelayakan_7kolom.csv",
    mime="text/csv",
)
